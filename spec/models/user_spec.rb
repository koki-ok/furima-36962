...
RSpec.describe User, type: :model do
  ...
  describe "ユーザー新規登録" do
    context "新規登録できるとき" do
      it "nickname,email,password,encrypted_password,last_name,first_name,last_name_kana,first_name_kana,birth_dayが存在すれば登録できる" do
    ...
    context "新規登録できないとき" do
      ...
      it "重複したemailが存在する場合は登録できない(一意性のテスト)" do
        @user.save
      ...
      it "last_name_kanaは全角カタカナ以外では登録できない" do
        ...
        @user.valid?
        expect(@user.errors.full_messages).to include("Last name kana は全角カタカナでの入力が必要です")
      end
      it "first_name_kanaは全角カタカナ以外では登録できない" do
        @user.first_name_kana = 'ほげ'
        @user.valid?
        expect(@user.errors.full_messages).to include("First name kana は全角カタカナでの入力が必要です")
      end
    end
  end
end
